...
/**
  ...
  */
function URLDecolarInvalidaError(message) {
...
function getDecolarDataURL (DecolarURL) {
  ...
  if(DECOLAR_HOST_REGEX.test(urlParsed.host) === false) {
    throw new URLDecolarInvalidaError('Host invalido - ' + urlParsed.host + ' - ' + DecolarURL);
  }
...
/**
  ...
  */
function RequestStatusNotOKError(message) {
...
function fetchDecolarData (DecolarDataURL) {
  ...
  request(DecolarDataURL, function (error, response, body) {
      ...
      if ( error ) {
        ...
        deferred.reject(new RequestStatusNotOKError('Decolar request retornou um StatusCode não OK - ' + response.statusCode));
      } else {
