...
function getDecolarDataURL (DecolarURL) {
  ...
  var dURL = DecolarURL.toLowerCase();

  // Adiciona prefixo "http://" se necessario
  var HTTP_PREFIX_REGEX = /http:\/\//;
  if (HTTP_PREFIX_REGEX.test(dURL) === false)
  {
    dURL = 'http://' + dURL;
  }

  // Parse URL
  var urlParsed = url.parse(dURL);

  // Verifica se a URL é valida
  if(validURL.isWebUri(dURL) === false) {
    var err = new URLDecolarInvalidaError('URL invalida.', DecolarURL);
    log.error(err);
    throw err;
  }
  ...
  if(DECOLAR_HOST_REGEX.test(urlParsed.host) === false) {
    var err = new URLDecolarInvalidaError('Host invalido.', DecolarURL);
    log.error(err);
    throw err;
  }
  ...
  dURL = urlParsed.format();

  // Verifica se a URL final é valida
  if(validURL.isWebUri(dURL) === false) {
    var err = new URLDecolarInvalidaError('URL final invalida.', DecolarURL);
    log.error(err);
    throw err;
  }
...
/**
  ...
  */
function RequestStatusNotOKError(message, statusCode) {
...
function fetchDecolarData (DecolarDataURL) {
  ...
  request(DecolarDataURL, function (error, response, body) {
      ...
      if ( error ) {
        ...
        var err = new RequestStatusNotOKError('Decolar request retornou um StatusCode não OK', response.statusCode);
        log.error(err);
        throw err;
      } else {
...
exports.getDecolarPreco = getDecolarPreco;
...
/**
  ...
  */
function scrapeDecolarPreco (DecolarURL) {
  return Q.fcall(getDecolarDataURL, DecolarURL)
    .then(fetchDecolarData)
    .then(JSON.parse)
    .then(getDecolarPreco);
}
exports.scrapeDecolarPreco = scrapeDecolarPreco;
